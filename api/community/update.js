/**
 * Community Agent Update API
 *
 * This serverless function handles updating agents in the community gallery.
...
module.exports = async function handler(req, res) {
    ...
    try {
        const { agentId, name, description, author, tags, data, adminPassword } = req.body;
        ...
        const originalAgent = currentContent.agents[agentIndex];

        // Update the agent with new data, preserving original data structure
        const updatedAgent = {
            ...originalAgent,
            name: name,
            description: description,
            author: author || originalAgent.author,
            tags: tags || originalAgent.tags,
            updatedAt: new Date().toISOString(),
            data: data || originalAgent.data
        };

        // Replace the agent in the array
        currentContent.agents[agentIndex] = updatedAgent;
        ...
        const updateResponse = await httpsRequest(getFileUrl, {
            ...
            message: `Update community agent: ${name}`,
            content: newContent,
            sha: sha,
            branch: 'main',
        }));
        ...
        if (!updateResponse.ok) {
            ...
            throw new Error('Failed to update agent in community');
        }
